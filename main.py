...
STOP_WORDS = {
    ...
    '不住', '不禁', '不觉', '不由得', '情不自禁',
    '小心', '提醒', '退后', '站住', '别动', '住手', '让开', '闭嘴', '快走', '慢着',
    '皱眉', '眯眼', '瞥见', '凝视', '远望', '四目相对', '面面相觑', '巡视', '环顾',
    '呼吸', '喘息', '叹气', '倒吸', '抽气', '哽咽',
    '冷笑', '苦笑', '浅笑', '微笑', '笑意', '咧嘴',
    '挥手', '握手', '摆手', '抬手', '放手', '拍肩', '撞肩',
    '蹙眉', '豁然', '释然', '恍惚', '晃神', '发愣', '怔住', '愣住', '愣神', '发呆',
    '若有所', '恍然大', '赫然', '愤然',
    '天色', '晨光', '午后', '傍晚', '夜幕', '暮色', '夕阳', '朝阳', '残阳', '月华',
    '日光', '月色', '乌云', '阴云', '薄雾', '雾霭', '浓雾', '寒风', '微风', '暖风',
    '热浪', '凉意', '冷意',
    '尘埃', '碎屑', '碎木', '碎石', '断壁残垣', '残垣断壁',
...
class SatrfateChatSearchPlugin(Star):
    ...
    async def on_llm_req(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        # ---- 1. 获取会话信息 ----
        session_id = event.message_obj.session_id  # 框架自动生成，格式: FriendMessage:{uid} 或 GroupMessage:{gid}
        ...
        if self.debug:
            logger.info(f"[ChatSearch] 暂存消息 [{session_id}] [{sender_name}]：{text[:40]}...")
        ...
        if injection_parts:
            ...
            if self.debug:
                logger.info(f"[ChatSearch] 注入 {len(history)} 条检索记忆，固定记忆 {len(fixed_content) if fixed_content else 0} 字符")
